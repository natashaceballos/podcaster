...
const PodcatsDetail: FC<{ podcast: SerieDetail }> = ({ podcast }) => {
  const {state:{description}} = usePodcatsDetail(podcast)
  return (
    <div className=" rounded-sm mx-auto shadow-card max-w-xs pr-2 h-fit break-words flex flex-col gap-y-3">
      ...
      <div className="flex flex-col">
        <Link to={`/podcast/${podcast.collectionId}`}>
          <p className="text-justify font-bold pl-2">{podcast.collectionName}</p>
        </Link>
      ...
      <div className=" flex flex-col pl-2">
        ...
        <div dangerouslySetInnerHTML={{ __html: description}} className='italic pb-5' />
      </div>
    </div>
  )
}
